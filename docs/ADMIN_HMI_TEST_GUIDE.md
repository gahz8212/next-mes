...
### ⑧ 품목 마스터 & BOM 관리 (`item` / `wo`의 BOM 등록 & 실시간 Diff 비교)
- **기능 설명**: 고객사별 생산 품목(PCB 기판) 및 표준 부품 명세서(BOM)를 관리합니다.
  - **거래처명 표시 & 연동**: 품목 목록의 첫 번째 컬럼에 해당 발주 고객사명(`LG전자`, `삼성` 등)이 표시되며, 품목 등록/수정 시 거래처를 선택하여 관리합니다.
  - **표준 BOM 상태 & [📋 BOM] 버튼**: 품목별 최신 버전(`v1.0 (4종 부품)`) 뱃지가 표시되며, 목록에서 `[📋 BOM]` 버튼을 눌러 언제든지 BOM을 열람·수정하거나 신규 품목 등록 직후 바로 BOM을 동시 등록할 수 있습니다.
  - **⚠️ 실시간 BOM Diff 비교 & 버전 선택 모달**: 기존 등록된 기준 BOM(v1.0)과 새로 입력된 BOM 간의 변경 사항(추가/삭제/수정/동일)을 실시간으로 정밀 비교 분석하여 차이점을 알려주고, 사용자에게 **`[신규 BOM 승인 및 v2.0 마스터 업데이트]`**, **`[이번 WO만 1회성 적용]`**, **`[기존 BOM 유지]`** 3가지 선택권을 제공합니다.
  - **`[✔ 사급 자재로 창고 입고 자동 등록]`(기본값: true)**: BOM 저장과 동시에 해당 고객사의 사급 자재 실물 입고까지 원클릭으로 자동 연계 처리됩니다.
- **백엔드 연동 API**: `get_items.php`, `create_item.php`, `update_item.php`, `get_bom.php`, `compare_bom.php`, `save_bom.php` (`auto_inbound: true`, `version`)
- **테스트 방법**:
  1. `품목 관리` 메뉴 접속 ➔ 첫 번째 열에 **`거래처명`**이 정상 표시되는지 확인.
  2. `[+ 품목 등록]` 클릭 ➔ 거래처 선택, 품목코드, 품목명 입력 후 `[저장]` ➔ *"지금 바로 BOM을 등록하시겠습니까?"* 팝업에서 `[확인]` 시 BOM 등록 모달 즉시 실행 확인.
  3. 특정 품목의 **`[📋 BOM]`** 버튼 클릭 ➔ 표준 부품 명세서 조회 및 엑셀 일괄 수정 동작 확인.
