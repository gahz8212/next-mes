...
- **기능 설명**: 생산 품목(PCB 기판)별 부품 명세서(BOM)를 엑셀 붙여넣기로 일괄 등록하고 마운터 피더 슬롯 매핑을 구성합니다.
...
- **백엔드 연동 API**: `get_items.php`, `create_item.php`, `get_bom.php`, `compare_bom.php`, `save_bom.php` (`auto_inbound: true`, `version`)
- **테스트 방법**:
  1. `작업지시 (WO) 관리`에서 특정 작업지시의 **`[BOM]`** 버튼 클릭.
  2. 수정된 엑셀 데이터(수량 변경 또는 신규 부품 추가)를 붙여넣고 `[BOM 저장 및 입고 처리]` 클릭.
  3. **`⚠️ BOM 변경 감지 모달`**이 자동으로 팝업되며, 변경된 부품(추가 `🟢`, 삭제 `🔴`, 수정 `🟡`)의 전/후 비교 테이블이 시각적으로 표시되는지 확인.
  4. `[신규 BOM을 최신 표준 버전으로 승인 (v2.0)]` 선택 후 `[선택한 BOM으로 진행 및 저장]` 클릭 ➔ 신규 버전 저장 및 피더 셋업 자동 반영 확인.
